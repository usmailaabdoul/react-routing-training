import { Link, useParams } from "react-router-dom";
import { useState, useEffect, useCallback, useMemo } from "react";

const headers = {
  "Content-Type": "Application/json",
};

const CharacterDetails = () => {
  let { characterId, characterName } = useParams();
  const [character, setCharacter] = useState(null);
  const [isLoading, setIsLoading] = useState(false);

  const fetchCharacter = useCallback(async () => {
    try {
      setIsLoading(true);
      let res = await fetch(`https://swapi.dev/api/people/${characterId}`, {
        headers,
        method: "GET",
      });
      res = await res.json();
      console.log({ res });
      setCharacter(res);
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
      console.log({ error });
    }
  }, [characterId]);

  useEffect(() => {
    fetchCharacter();
  }, [fetchCharacter]);

  const renderFilm = useCallback(async (film) => {
    let filmName = ''
    try {
      let res = await fetch(`${film}`, {
        headers,
        method: "GET",
      });
      res = await res.json();
      console.log({ res });
      filmName = res.title;
    } catch (error) {
      console.log({ error });
    }  
    
    if (filmName) {
      return <div className="bg-neutral rounded-sm p-2">{filmName}</div>
    }
  }, []);

  return (
    <div className="container px-32 flex-1 mt-10">
      {isLoading && (
        <div class="text-center">
          <div role="status">
            <svg
              aria-hidden="true"
              class="inline w-12 h-12 text-gray-200 animate-spin fill-primary"
              viewBox="0 0 100 101"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                fill="currentColor"
              />
              <path
                d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                fill="currentFill"
              />
            </svg>
            <span class="sr-only">Loading...</span>
          </div>
        </div>
      )}

      {!isLoading && character && (
        <div className="relative inline-flex group w-full">
          <div class="absolute transitiona-all duration-1000 opacity-50 -inset-px bg-gradient-to-r from-[#44BCFF] via-[#FF44EC] to-[#FF675E] rounded-xl blur-lg group-hover:opacity-100 group-hover:-inset-1 group-hover:duration-200 animate-tilt"></div>
          <div className="flex bg-dark relative w-full">
            <img
              className="rounded-t-sm w-[250px] h-[340px]"
              src={require(`../../assets/img/characters/${characterId}.jpg`)}
              alt={character.name}
            />
            <div className="border-neutral rounded-sm p-5 flex gap-20">
              <div>
                <div className="font-mono text-2xl mb-2">{character.name}</div>
                <div className="flex gap-2 mb-1 font-mono">
                  <div>Birth Year:</div>
                  <div>{character.birth_year}</div>
                </div>
                <div className="flex gap-2 mb-1 font-mono">
                  <div>Species:</div>
                  <div>{character.species.length > 0 ? "Alien" : "Unknown"}</div>
                </div>
                <div className="flex gap-2 mb-1 font-mono">
                  <div>Height:</div>
                  <div>{character.height}</div>
                </div>
                <div className="flex gap-2 mb-1 font-mono">
                  <div>Mass:</div>
                  <div>{character.mass}</div>
                </div>
                <div className="flex gap-2 mb-1 font-mono">
                  <div>Gender:</div>
                  <div>{character.gender}</div>
                </div>
                <div className="flex gap-2 mb-1 font-mono">
                  <div>Hair Color:</div>
                  <div>{character.hair_color}</div>
                </div>
              </div>
              <div className="h-auto">
                <div className="font-mono text-xl mb-2">Films:</div>
                <div className="flex flex-wrap gap-3 hover:cursor-pointer">
                  {character.films.map((film) => (
                    <Film key={film} film={film} />
                  ))}
                </div>
              </div>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default CharacterDetails;

const Film = ({film}) => {
  const filmNum = film.split('/')[film.split('/').length - 2];
  return (
    <div className="w-[150px]">
      <img className="rounded-t-sm" src={require(`../../assets/img/films/${filmNum}.jpg`)} alt={filmNum} />
    </div>
  )
}